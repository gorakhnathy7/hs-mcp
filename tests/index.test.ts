...
import { APIPromise } from 'hyperswitch/core/api-promise';

import util from 'node:util';
import Hyperswitch from 'hyperswitch';
import { APIUserAbortError } from 'hyperswitch';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['HYPERSWITCH_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Hyperswitch) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['HYPERSWITCH_LOG'] = 'debug';
      const client = new Hyperswitch({
        logger: logger,
        apiKey: 'My API Key',
        ephemeralKey: 'My Ephemeral Key',
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['HYPERSWITCH_LOG'] = 'not a log level';
      const client = new Hyperswitch({
        logger: logger,
        apiKey: 'My API Key',
        ephemeralKey: 'My Ephemeral Key',
      ...
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'HYPERSWITCH_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['HYPERSWITCH_BASE_URL'] = undefined;
    });
    ...
    test('env variable', () => {
      process.env['HYPERSWITCH_BASE_URL'] = 'https://example.com/from_env';
      const client = new Hyperswitch({
        apiKey: 'My API Key',
        ephemeralKey: 'My Ephemeral Key',
        jwtKey: 'My Jwt Key',
    ...
    test('empty env variable', () => {
      process.env['HYPERSWITCH_BASE_URL'] = ''; // empty
      const client = new Hyperswitch({
        apiKey: 'My API Key',
        ephemeralKey: 'My Ephemeral Key',
        jwtKey: 'My Jwt Key',
      ...
      expect(client.baseURL).toEqual('https://sandbox.hyperswitch.io');
    });

    test('blank env variable', () => {
      process.env['HYPERSWITCH_BASE_URL'] = '  '; // blank
      const client = new Hyperswitch({
        apiKey: 'My API Key',
        ephemeralKey: 'My Ephemeral Key',
        jwtKey: 'My Jwt Key',
    ...
    test('in request options overridden by env variable', () => {
      process.env['HYPERSWITCH_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Hyperswitch({
        apiKey: 'My API Key',
        ephemeralKey: 'My Ephemeral Key',
        jwtKey: 'My Jwt Key',
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['HYPERSWITCH_API_KEY'] = 'My API Key';
    process.env['HYPERSWITCH_EPHEMERAL_KEY'] = 'My Ephemeral Key';
    process.env['HYPERSWITCH_JWT_KEY'] = 'My Jwt Key';
    process.env['HYPERSWITCH_PUBLISHABLE_KEY'] = 'My Publishable Key';
    const client = new Hyperswitch();
    expect(client.apiKey).toBe('My API Key');
    expect(client.ephemeralKey).toBe('My Ephemeral Key');
    expect(client.jwtKey).toBe('My Jwt Key');
    expect(client.publishableKey).toBe('My Publishable Key');
  });

  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['HYPERSWITCH_API_KEY'] = 'another My API Key';
    process.env['HYPERSWITCH_EPHEMERAL_KEY'] = 'another My Ephemeral Key';
    process.env['HYPERSWITCH_JWT_KEY'] = 'another My Jwt Key';
    process.env['HYPERSWITCH_PUBLISHABLE_KEY'] = 'another My Publishable Key';
    const client = new Hyperswitch({
      apiKey: 'My API Key',
      ephemeralKey: 'My Ephemeral Key',
      jwtKey: 'My Jwt Key',
